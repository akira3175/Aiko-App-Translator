...
export function createPublishingBooksFeature({api,escapeHtml,getProject,getRevision=()=>0,toast}) {
  let books=[];
  ...
  const load=async({strict=false}={})=>{
    ...
    try{const data=await api('/api/publishing?project='+encodeURIComponent(project));if(project!==getProject()||revision!==getRevision())return;books=data.books||[];render();}
    catch(error){if(project!==getProject()||revision!==getRevision())return;if(strict)throw error;books=[];render();toast(error.message);}
  };
  const save=async()=>{
