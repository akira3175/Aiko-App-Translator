  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {(notice || error) && <div role="status" className={error ? "message error" : "message success"}>{error || notice}<button aria-label="Đóng thông báo" onClick={() => { setError(""); setNotice(""); }}><X size={18} aria-hidden="true" /></button></div>}

        {!project ? <EmptyProject onCreate={() => void createProject()} onImport={() => novelImportRef.current?.click()} importing={importingNovel} /> : view === "translate" ? (
          <TranslateView
            chapters={chapters}
            chapter={chapter}
            chapterId={chapterId}
...
function ManualTranslationModal({ value, onChange, onCancel, onConfirm }: { value: ManualTranslationDraft; onChange: (value: ManualTranslationDraft) => void; onCancel: () => void; onConfirm: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}><section className="manual-translation-modal" role="dialog" aria-modal="true" aria-labelledby="manual-translation-title"><div className="chapter-import-heading"><div><p className="eyebrow">DỊCH THỦ CÔNG</p><h2 id="manual-translation-title">Đưa prompt sang AI bên ngoài</h2><p>Sao chép prompt đầy đủ, sau đó dán nguyên kết quả có các marker TITLE và CONTENT.</p></div><button className="modal-close" aria-label="Đóng" onClick={onCancel}><X size={18} aria-hidden="true" /></button></div><div className="manual-translation-steps"><label><span>Bước 1 · Prompt đầy đủ</span><textarea readOnly spellCheck={false} value={value.prompt} /></label><button className="button outline manual-copy-prompt" onClick={() => void copyPrompt()}><ClipboardText size={17} />{copied ? "Đã sao chép" : "Sao chép prompt"}</button><label><span>Bước 2 · Dán toàn bộ kết quả AI</span><textarea autoFocus spellCheck={false} placeholder={"###TITLE###\nTiêu đề đã dịch\n\n###CONTENT###\nNội dung đã dịch\n\n###END###"} value={value.result} onChange={(event) => onChange({ ...value, result: event.target.value })} /></label></div><div className="modal-actions"><button className="button outline" onClick={onCancel}>Hủy</button><button className="button primary" disabled={!value.result.trim()} onClick={onConfirm}>Lưu bản dịch</button></div></section></div>;
}

function EmptyProject({ onCreate, onImport, importing }: { onCreate: () => void; onImport: () => void; importing: boolean }) {
  return <div className="empty-state"><div className="empty-icon"><BookOpenText size={30} /></div><h2>Chưa có dự án</h2><p>Tạo dự án mới hoặc nhập EPUB/TXT để bắt đầu dịch ngay trong trình duyệt.</p><div className="empty-actions"><button className="button primary" onClick={onCreate}><Plus size={18} />Tạo dự án đầu tiên</button><button className="button outline" disabled={importing} onClick={onImport}><FilePlus size={18} />{importing ? "Đang nhập..." : "Nhập EPUB / TXT"}</button></div></div>;
}
    ...
    <div className="editor-workspace">
      {chapter ? <>
        ...
        <div className="editor-grid">
          <section className={mobilePane === "source" ? "mobile-pane-active" : ""}><div className="pane-label"><strong className="editable-pane-title">BẢN GỐC<small>{busy ? "TÁC VỤ ĐANG CHẠY · CHỈ ĐỌC" : "SOẠN THẢO · TỰ ĐỘNG LƯU"}</small></strong><div><span>{chapter.source.length.toLocaleString("vi-VN")} ký tự</span><button disabled={!!busy} type="button" onClick={() => openEditorFind("source")}><MagnifyingGlass size={14} />Tìm</button><button disabled={!!busy} className={sourcePreview ? "active" : ""} type="button" onClick={() => setSourcePreview((value) => !value)}>{sourcePreview ? <Eye size={15} /> : <PencilSimple size={15} />}{sourcePreview ? "Bản đọc" : "Markdown"}</button></div></div>{sourcePreview ? <MarkdownPreview editorStyle label="Xem trước chương nguồn" value={chapter.source} assets={assets} onEditLine={(line) => editPreviewLine("source", line)} /> : <MarkdownEditor ref={sourceEditorRef} readOnly={!!busy} label="Nội dung chương nguồn có thể chỉnh sửa" value={chapter.source} onChange={(source) => onUpdate({ source })} />}</section>
